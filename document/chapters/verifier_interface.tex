...
{Does the verifier support ONNX networks with multiple input or output nodes? See Section~\ref{sec:input-output-declarations} for details.}
...
\clOutputOption
{--multiple-networks}
{Does the verifier support queries with multiple networks? See Section~\ref{sec:multi-network-declarations} for how to declare multiple networks in a query.}
{A boolean (\texttt{true} or \texttt{false})}
\begin{lstlisting}[style=bash]
...
\clOutputOption
{--hidden-nodes}
{Does the verifier support queries which refer to hidden nodes in the network? See Section~\ref{sec:hidden-node-declarations} for how to declare hidden nodes in a query.}
{A boolean (\texttt{true} or \texttt{false})}
\begin{lstlisting}[style=bash]
