...
\end{itemize}
Arithmetic operations only operate over individual elements of tensors and do not operate over multi-dimensional tensors (see Section~\ref{sec:scoping_and_typing} for detailed typing rules). 
...
Crucial to the typing rules is the notion of the current \emph{context} $\assertCtx = (\networkCtx, \variableCtx)$ which consists of two parts. The first is the \emph{network context}, $\networkCtx : \mgrammar{<name>} \rightarrow \mgrammar{<network>}$, which is a mapping from network names to the corresponding network declarations. The second is the \emph{variable context}, $\variableCtx : \mgrammar{<name>} \rightarrow \mgrammar{<tensorType>}\networkTheoryVarParam$, which is a mapping from the decared input/hidden/output variables to their shape and type.
...
\input{diagrams/typing-rule-model-query.tex}
...
\input{diagrams/typing-rule-model-nil-cons.tex}
...
\input{diagrams/typing-rule-model-network.tex}
...
\input{diagrams/typing-rule-model-iso.tex}
...
\input{diagrams/typing-rule-model-hidden.tex}
