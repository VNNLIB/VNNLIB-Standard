...
% \textit{arithmetic} \(\langle\) \text{arith} \(\rangle\), \textit{comparative} given by \(\langle \text{comp} \rangle\)$, and \textit{boolean} given by \(\langle \text{bool} \rangle\),

% Types \langle \textit{type} \rangle are used to define the element type of variables, while expressions are used to define the values of variables.
