...
At the centre of the \vnnlib{} standard is the \vnnlib{} query language. Heavily inspired by the query language from SMT-LIB, it is designed as a standardised computer-readable format for expressing satisfiability problems over neural networks.
Although the syntax is somewhat human-readable, the language is designed to primarily optimise machine-readability. 
...
\begin{figure}
	...
	\end{grammar}
	\vspace{-1em}
    \caption{The syntax of \vnnlib{} queries, parameterised by the syntax of the underlying network theory $\networkTheoryVar$ described in Figures~\ref{fig:onnx-type-syntax}~\&~\ref{fig:onnx-expr-syntax}. The superscript $^*$ means zero or more, $^+$ means one or more, and $^?$ means optional.}
    \label{fig:vnnlib-syntax}
\end{figure}

The syntax of the \vnnlib{} query language is shown in Figure~\ref{fig:vnnlib-syntax}.  This section describes the key syntactic constructs of the language primarily via illustrative examples. Firstly, a query consists of three parts: 
\begin{enumerate}
...
\begin{itemize}
    ...
    \item \inlinevnn{(- a b ...)}: Subtraction of two or more terms. The operation associates to the left, e.g. \inlinevnn{(- 1 2 3 4)} is the same as \inlinevnn{(- (- (- 1 2) 3) 4)} .
\end{itemize}
Arithmetic operations only operate over individual elements of tensors and do not operate over multi-dimensional tensors (see Section~\ref{sec:typing} for detailed typing rules). 
...
It is permitted to declare zero-dimensional network nodes, i.e. a node that takes scalar values. 
...
In particular, the empty list of the networks is always well-typed under any context and returns that very same context. A non-empty list of networks is well-typed if the head of the list can be typed under the current context, and the tail is well-typed under the current context extended by the head of the list.
...
Finally, if the network is marked as isomorphic to some other network in the context then the shapes, not the element types, of the input and output declarations for the two network have to match:
...
As input, solvers receive a query $\queryVar$ and a list of ONNX models $\networkImplementationSet = \networkImplementations$. 
This section describes a series of judgements that formalise the notion of $\networkImplementationSet$ being compatible with the networks declared by $\queryVar$. 
The context, $\newImplementationCtx$, used in these judgments is a map from network declaration names to network models.

To start with, a list of ONNX models $\networkImplementationSet$ are compatible with a query $\queryVar$ if they can be judged to be compatible with the list of network declarations in the empty context. 
\input{diagrams/typing-rule-model-query.tex}\unskip
A list of models is judged to be compatible with a list of network declarations if each model is compatible with the corresponding network declaration, with the models progressively being added to the context.
\input{diagrams/typing-rule-model-nil-cons.tex}\unskip
An individual model $\modelVar$ is compatible with a network declaration $\networkDeclVar$ if (i) it is compatible with the equivalence statement, (ii) it can be typed according according to the network declaration and (iii) it has hidden nodes of the correct type: 
...
A network model is compatible with $\texttt{isomorphic-to }\nameVar$ and $\texttt{equal-to }\nameVar$ statements if it related to the network model provided for the earlier referenced network declaration $\nameVar$ in the appropriate way:
\input{diagrams/typing-rule-model-iso.tex}\unskip
If no equivalence statement is provided, then any network implementation is compatible. Note that the equality judgment `$=$' in the $\texttt{equal-to }\nameVar$ case is \emph{not} parameterised by $\networkTheoryVar$ as it refers to absolute equality, i.e. the network models must actually be the \emph{same} model file. 

Finally a network model is compatible with a list of hidden node declarations if each hidden node declaration corresponds to a hidden node in the model of the correct type:
...
\input{diagrams/typing-rule-inputAssignment.tex}\unskip
...
\end{equation*}
that maps each name $\nameVar$ of the declared input/hidden/output network variable of type $\tensorTypeVar$ to mathematical tensors of the corresponding type.

Given a list of network declarations $\networkDeclSet$, a list of models $\networkImplementationSet$ and an input assignment $\networkInputSet$, the environment $\assertEnv (\networkDeclSet, \networkImplementationSet, \networkInputSet)$ is calculated as the union of the environments generated by each network:
\begin{equation*}
...
\begin{align*}
    ...
    \\
    \assertEnv_{\inputDeclSet} 
    &= \{ \nameVar \rightarrow \semTensorTheory{\networkInput_i}
    \mkern94mu \mid 1 \leq i \leq |\inputDeclSet| \} 
    \mkern15mu \text{ where $\inputDeclSet_i = \inputExpansion$}
    \\
    \assertEnv_{\hiddenDeclSet} 
    &= \{ \nameVar \rightarrow \semModelHiddenTheory{\networkImplementation, \onnxNameVar}(\sem{\networkInput}) 
    \mid 1 \leq i \leq |\hiddenDeclSet|) \}
    \text{ where $\hiddenDeclSet_i = \hiddenExpansion$}
    \\  
    \assertEnv_{\outputDeclSet} 
    &= \{ \nameVar \rightarrow \semModelTheory{\networkImplementation}(\sem{\networkInput})_i
    \mkern56mu \mid 1 \leq i \leq |\outputDeclSet| \}
    \mkern10mu \text{ where $\outputDeclSet_i = \outputExpansion$}
\end{align*}}
