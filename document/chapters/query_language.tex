...
\begin{figure}
    ...
    \begin{minipage}[t]{1\textwidth}
        \centering
            ...
            $\langle$NetworkDeclaration$\rangle$ & $\ni$ & $D ::= (\texttt{declareNetwork}~v~C_?~I^+~H^*~O^+$) \\ \\
    ...
    \begin{minipage}[t]{1\textwidth}
        ...
        \begin{mathpar}
            \inferrule*[Right=InputDeclaration]{\;}{
                \Gamma \vdash (\texttt{declareInput}~v~\tau~s) : \Gamma, v \rightarrow (\tau,s) 
            }
            \\
            \inferrule*[Right=HiddenDeclaration]{\;}{
                \Gamma \vdash (\texttt{declareHidden}~v~\tau~s) : \Gamma, v \rightarrow (\tau,s)
            }
            \\
            \inferrule*[Right=OutputDeclaration]{\;}{
                \Gamma \vdash (\texttt{declareOutput}~v~\tau~s) : \Gamma, v \rightarrow (\tau,s)
            }
            \\ \\
            \inferrule*[Right=NetworkDeclaration]{
                ...
                \Gamma_0 \vdash (\texttt{declareNetwork}~v~C_?~(I_1~...~I_n)^+~(H_1~...~H_m)^*~(O_1~...~O_o)^+) : \Gamma_{n+m+o}
            }
            \end{mathpar}
        }
    \caption{Typing judgement of a Network Declaration}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \begin{minipage}[t]{1\textwidth}
        \resizebox{\textwidth}{!}{
        \begin{mathpar}
            \inferrule*[Right=ArithExpr Double Real]{
                d \in \mathbb{Q} %using rational instead of real
            }{
                \Gamma \vdash d : \text{Real}
            }
            \\
            \inferrule*[Right=ArithExpr Double Float32]{
                d \in \text{Float32}
            }{
            ...
            \vdots
            \\
            \inferrule*[Right=ArithExpr Variable]{
                \Gamma [v] : (\tau,s) \quad \text{validIndices} (i, s)
            }{
                \Gamma \vdash v~i : \tau
            }
            \\
            \inferrule*[Right=ArithExpr Negation]{
                \Gamma \vdash a : \tau
            }{
                \Gamma \vdash (\texttt{-}~a) : \tau
            }
            \\
            \inferrule*[Right=ArithExpr N-ary Operations]{
                \Gamma \vdash a_1 : \tau \quad ... \quad \Gamma \vdash a_n : \tau
            }{
                \Gamma \vdash (\circledast~a_1~...~a_n) : \tau \quad \circledast \in \{\texttt{+}, \texttt{-}, \texttt{×}\}
            }
        \end{mathpar}
        }
    ...
    \begin{minipage}[t]{1\textwidth}
        ...
        \begin{mathpar}
            \inferrule*[Right=BoolExpr Comparisons]{
                ...
                \Gamma \vdash (\lozenge~a_1~a_2) \quad \lozenge \in \{\texttt{>=}, \texttt{>}, \texttt{<}, \texttt{<=}, \texttt{==}, \texttt{!=}\}
            }
            \\
            \hspace{1cm}
            \inferrule*[Right=BoolExpr Connectives]{
                \Gamma \vdash b_1 \quad ... \quad \Gamma \vdash b_n
            }{
                \Gamma \vdash (\Box~b_1~...~b_n) \quad \Box \in \{\texttt{and}, \texttt{or}\}
            }
            \\
            ...
            \inferrule*[Right=Query]{
                \ensuremath{
                    ...
                    \vdots & \vdots \\
                    \Gamma_{n-1} \vdash D_n : \Gamma_n &  \vdash A_m
                    \end{array}
                }
            }{
                (V~(D_0~...~D_n)~(A_0,...,A_m))
...
Given that the actual semantics of a \vnnlib{} query is dependent on the semantics of the neural network representation i.e. on the underlying ONNX model(s) it is associated with, we can describe the semantics of a query as a type of higher-order satisfiability problem. 
...
For semantics, the semantic context is constructed from the Declarations of a \vnnlib{} query, which represents each network as a function from a set of inputs to a set of outputs, and storing this \textit{network implementation} alongside the set of inputs.
