...
The static context \(\Gamma\) is the environment that contains and ordered list of network functions and inputs . The environment function \(Sigma\) gives the type of the variable by fetching this from the context. The typing rules are defined in Figure~\ref{fig:vnnlib_typing_rules}.
...
\begin{figure}
    ...
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \begin{flalign*}
            & \langle expr \rangle &\ni e ::= x \;|\; f  \;|\; i &\in \mathbb{N} \;|\; b &\in \mathbb{B} && \\
            & \;|\; \lor \;|\; \land && \\
            & \;|\; > \;|\; < \;|\; >= \;|\; <= \;|\; == \;|\; \neq && \\
            & \;|\; \times \;|\; + \;|\; - \;|\; && \\
            & \;|\; ! && \\ \\
            & \langle type \rangle \ni \tau ::= s \rightarrow s \;|\; \tau && \\
            & \langle onnx-types \rangle &\ni o ::= \text{the set of all ONNX types} && \\
            & \langle tensor \rangle &\ni t ::= \text{Tensor} \ [n_0, ..., n_n] \ \quad n &\in \mathbb{N} && \\
            & \langle basic-types \rangle &\ni s ::= \text{Bool} \;|\; \text{Comp} \ o \;|\; \text{Arith} \ o \;|\; \text{Index} \ n \quad n &\in \mathbb{N} \\
            & \langle complex-type \rangle &\ni c ::= \text{List } n \ s \;|\; \text{List } n \ t \quad n &\in \mathbb{N} && \\
        \end{flalign*}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
    ...
    \end{minipage}
    \caption{The \vnnlib{} query language typing syntax and rules}
