...
\subsubsection{Tensors with zero dimensions}

It is permitted to declare zero-dimensional input, output and hidden variables that represent scalar values. 
In this case, such variables should be referenced without any indices, as shown in Line 8 below:
\begin{lstlisting}[style=lbnf]
(declare-network f
...
\subsubsection{Tensors with dynamic dimensions}

The ONNX standard supports tensors with dynamic dimensions. 
Named dimensions are stored as a string in the model, while unnamed dimensions are stored as a null value in the model but are often written as ``-1'' in mathematics for clarity\footnote{If you actually set a dimension size to ``-1'' in the model it is treated as named dimension.}. 
For example, a model with an input tensor with dimensions $[batch, -1, 28, 28]$ accepts any 4-D tensor where the last two dimensions must be $[28, 28]$ but the first two dimensions can be any size.

It is not currently possible to explicitly write dynamic dimensions in a \vnnlib{} query. 
Instead, the query must instantiate the dynamic dimensions to concrete values. 
For example, if the ONNX model has an input with the dimensions $[batch, -1, 28, 28]$ then the corresponding network declaration could be:
\begin{lstlisting}[style=lbnf]
(declare-network f
  (declare-input X float32 [1,3,28,28])
  ...
)
\end{lstlisting}
where first dynamic dimension named $batch$ has been set to 1, and second unnamed dynamic dimension has been set to 3.
