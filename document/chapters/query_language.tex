    ...
    \begin{minipage}[t]{1\textwidth}
        ...
        \begin{mathpar}
            \inferrule*[Right=InputDeclaration]{
                \mu = \texttt{input} \quad \mu \in \{\texttt{input}, \texttt{hidden}, \texttt{output}\}
            }{
                \Gamma \vdash (\texttt{declare-input}~v~\tau~s) : \Gamma, v \rightarrow (\tau,s,\mu) 
            }
            \\
            \inferrule*[Right=HiddenDeclaration]{
                \mu = \texttt{hidden} \quad \mu \in \{\texttt{input}, \texttt{hidden}, \texttt{output}\}
            }{
                \Gamma \vdash (\texttt{declare-hidden}~v~\tau~s) : \Gamma, v \rightarrow (\tau,s,\mu)
            }
            \\
            \inferrule*[Right=OutputDeclaration]{
                \mu = \texttt{output} \quad \mu \in \{\texttt{input}, \texttt{hidden}, \texttt{output}\}
            }{
                \Gamma \vdash (\texttt{declare-output}~v~\tau~s) : \Gamma, v \rightarrow (\tau,s,\mu)
            }
            \\ \\
    ...
    \begin{minipage}[t]{1\textwidth}
        ...
        \begin{mathpar}
            ...
            \inferrule*[Right=Variable]{
                \Gamma [v] : (\tau,s,\mu) \quad \text{validIndices}~(i, s)\quad\text{where}~\mu~\in~\{\texttt{input}, \texttt{hidden}, \texttt{output}\}
            }{
                \Gamma \vdash v~i : \tau
            }
            \\
...
\begin{align}
    \semenv{\Gamma\vdash{d}} &= d \\
    \semenv{\Gamma\vdash{v~i}} &= 
    \begin{cases}
        \Delta_{A_i}^k\quad\text{if}~g~\equiv (\tau,s,\texttt{input})\\
        \Delta_{N_k} (\Delta_{A_i}^k)\quad\text{if}~g~\equiv (\tau,s,\texttt{output})\\
    \end{cases} \\
