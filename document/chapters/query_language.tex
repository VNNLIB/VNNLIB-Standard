...
\begin{figure}
    ...
    \begin{minipage}[t]{1\textwidth}
        \centering
            ...
            $\langle$VariableName$\rangle$ & $\ni$ & $v ::= \verb|^[A-Za-z][A-Za-z0-9\_]*$| \quad \text{(regex)}$ \\
            ...
            $\langle$Version$\rangle$ & $\ni$ & $V ::= \verb|^\(vnnlib-version\s+<[0-9]+\.[0-9]+>)$| \quad \text{(regex)}$ \\
            $\langle$Query$\rangle$ & $\ni$ & $Q ::= V~D^*~A^+$ \\
            \end{tabular}
    ...
    \begin{minipage}[t]{1\textwidth}
        ...
        \begin{mathpar}
            ...
            \inferrule*[Right=NetworkDeclaration]{
                \ensuremath{
                    \begin{array}{ccc}
                    \Gamma_0 := \Gamma \vdash I_0 & \Gamma_{n+1} := \Gamma_n \vdash H_0 & \Gamma_{m+n+1} := \Gamma_{m+n} \vdash O_o \\
                    \vdots & \vdots & \vdots \\
                    \Gamma_n := \Gamma_{n-1} \vdash I_n & \Gamma_{m+n} := \Gamma_{m+n-1} \vdash H_m & \Gamma_{n+m+o} := \Gamma_{m+n+o-1} \vdash O_o
                    \end{array}
                }
            }{
                \Gamma \vdash (\texttt{declareNetwork}~v~c_?~(I_0~...~I_n)^+~(H_0~...~H_m)^*~(O_0~...~O_o)^+) : \Gamma_{n+m+o}
            }
            \\ \\
            \inferrule*[]{\;}{
                \Gamma \Vdash d : \tau
            ...
            \hspace{1cm}
            \inferrule*[]{
                \Gamma [v] : (\tau,s) \quad \text{validIndices} (j, s)
            }{
                \Gamma \Vdash v~j : \tau
            }
            \hspace{1cm}
            ...
            \inferrule*[Right=Assertion]{
                ...
                \Gamma \Vdash (\texttt{assert}~b) : \mathbf{Bool}
            }
            \\ \\
            \inferrule*[Right=Query]{
                \ensuremath{
                    \begin{array}{ccc}
                    \Gamma_0 := \Gamma := \Gamma_0 \vdash D_0 & \Gamma_n \Vdash A_0 : \mathbf{Bool} \\
                    \vdots & \vdots \\
                    \Gamma_n := \Gamma_n := \Gamma_{n-1} \vdash D_n & \Gamma_n \Vdash A_m : \mathbf{Bool}
                    \end{array}
                }
            }{
                \Gamma \Vdash (V~(D_0~...~D_n)~(A_0,...,A_m)) : \mathbf{Bool}
            }
        \end{mathpar}
        }
