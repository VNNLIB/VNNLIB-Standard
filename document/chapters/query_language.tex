...
Declarations are used to construct the static context $\Gamma$ which is used to type the variables and literals used in the Assertions of a query.
...
\begin{figure}
    ...
    \begin{minipage}[t]{1\textwidth}
        ...
        \resizebox{\textwidth}{!}{
            ...
            $\langle$InputDeclaration$\rangle$ & $\ni$ & $I::= \texttt{declareInput}~v~e~t$ \\
            $\langle$HiddenDeclaration$\rangle$ & $\ni$ & $H ::= \texttt{declareHidden}~v~e~t$ \\
            $\langle$OutputDeclaration$\rangle$ & $\ni$ & $O ::= \texttt{declareOutput}~v~e~t$ \\
            $\langle$NetworkDeclaration$\rangle$ & $\ni$ & $D ::= \texttt{declareNetwork}~v \quad [I_0,...,I_n]^+ ~[H_0,...,H_n] ~[O_0,...,O_n]^+ $ \\ \\
            $\langle$Double$\rangle$ & $\ni$ & $d ::= \mathbb{Q}$ \\
            $\langle$TensorIndex$\rangle$ & $\ni$ & $i ::= [i_0, ..., i_n]^+ \quad i \in \mathbb{N}$ \\
            ...
            $\langle$BoolExpr$\rangle$ & $\ni$ & 
                \begin{tabular}[t]{@{}l@{}}
                $b ::= c$ \\
                $\quad\;|\; \texttt{and}~c_0~...~c_n$ \\
                $\quad\;|\; \texttt{or}~c_0~...~c_n$
                \end{tabular} \\
