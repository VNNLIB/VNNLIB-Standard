...
At the centre of the \vnnlib{} standard is the \vnnlib{} query language. Directly inspired by the SMT-LIB query language, it is designed as a format for expressing satisfiability problems over neural networks.
Although the syntax is somewhat human-readable, the language is primarily designed to be machine-readable. 
It is expected that the vast majority of \vnnlib{} queries will be generated automatically by higher level tools that provide more human-friendly interfaces.
This chapter describes the syntax, scoping, typing and semantics of the query language, and, unless explicitly stated otherwise, everything in this chapter is implicitly parameterised by some underlying network theory~$\networkTheoryVar$ from Section~\ref{sec:network-theory}.
...
\begin{figure}
	...
	\input{diagrams/vnnlib-syntax.tex}
	\vspace{-1em}
    \caption{The syntax of \vnnlib{} queries. The superscript $^*$~means zero or more, $^+$~means one or more, $^?$~means optional and $\networkTheoryVarParam$~means the piece of syntax is defined by the underlying network theory $\networkTheoryVar$ as described in Figures~\ref{fig:onnx-type-syntax}~\&~\ref{fig:onnx-expr-syntax}.}
    \label{fig:vnnlib-syntax}
\end{figure}

The syntax of the \vnnlib{} query language is shown in Figure~\ref{fig:vnnlib-syntax}. At the top-level a \vnnlib{} query consists of three parts: 
\begin{enumerate}
\item \textbf{Version} - the version of the VNN-LIB the query is using. This allows tools to provide better diagnostic error messages.
...
\end{enumerate}
The standard does not permit the interleaving of network declarations and assertions. Figure~\ref{fig:simple-query} shows an example of a simple query.
...
All variable names follow the same syntax conventions: they are case-sensitive, must start with a letter, and may only contain letters, digits and underscores. 
...
(assert (<= Y[0,1] 0.5))\end{lstlisting}
    ...
    \end{minipage}
    \caption{A simple \vnnlib{} query which declares a network with a single input and output tensor. An example of one of the many possible ONNX models compatible with this declaration is shown on the right. Note that the variable names \texttt{X} and \texttt{Y} for the declared inputs and outputs do not have to match the node names in the ONNX model.}
    \label{fig:simple-query}
\end{figure}


It is important to emphasise that a network declaration only declares an \emph{interface} and does not reference any particular ONNX network model. 
The name of the network in the \vnnlib{} query is not required to match the name of the ONNX model file passed to the solver. 
Neither are the names of the inputs and outputs declared in the \vnnlib{} query required to match the names of the input and output nodes within the ONNX model file. 
This flexibility allows many different alternative ONNX models to be compatible with the same query.
As described in Section~\ref{sec:verify_command}, the user will explicitly associate a network declaration to the relevant model file via the command line.

The network declarations also do not constrain the internal architecture of the model, including which operators  are used (e.g. Gemm, ReLU, Softmax). In practice, different solvers will support different operators and Section~\ref{sec:onnx-capabilities} provides a mechanism for a solver to report the extent of its support.
...
They use an SMT-LIB-like reverse Polish notation with the operator preceding its operands.
An assertion is a logical formula that may include logical connectives, relational comparisons, and arithmetic expressions over declared variables and constants.
...
\begin{itemize}
    ...
    \item \inlinevnn{(- a b ...)}: Subtraction of two or more terms. The operation associates to the left, e.g. \inlinevnn{(- 1 2 3 4)} is the same as \inlinevnn{(- (- (- 1 2) 3) 4)} which is the same as \inlinevnn{(- 1 (+ 2 3 4))}.
\end{itemize}
Arithmetic operations only operate over individual elements of tensors rather than multi-dimensional tensors. See Section~\ref{sec:typing} for detailed typing rules.
...
\end{itemize}
There is no operator corresponding to negation, as it can always be eliminated by pushing it inwards to the assertion level.
...
(declare-network multi_io_net
    ...
    \caption{A network declaration with multiple inputs/outputs.}
...
The query language supports such networks by allowing a network declaration to contain an arbitrary number of input and output declarations. 
As will be described more formally in Section~\ref{sec:model-typing}, the declared inputs and outputs must exactly match the provided ONNX model's inputs and outputs. Concretely, this means:
\begin{enumerate}
...
This feature is not currently planned as it (a) would not increase the expressive power of the query language, (b) would complicate the implementation of the solver, (c) would complicate the semantics of the query language and (d) would make it possible for the user to unintentionally omit inputs from the network declaration.

\subsubsection{Inputs with initializers}
\label{sec:inputs-with-initializers}

In an ONNX model, an input may have a corresponding initializer that provides a default value for that input (see the \href{https://onnx.ai/onnx/repo-docs/IR.html}{ONNX IR specification}).
Inputs with initializers must still be listed in the network declaration.

By default, an input with an ONNX initializer is treated like any other input: the solver will not automatically assign it the initializer value, and instead the user must manually constrain it to the initializer value using standard equality assertions if desired.

The \texttt{initialized} keyword may optionally be appended to an input declaration to indicate that the solver should automatically set that input to the value specified by the corresponding initializer in the ONNX model.
Figure~\ref{fig:initialized-input} shows an example of a network declaration with an initialized input.
...
(declare-network multi_io_net
    ...
    \caption{A network declaration where the \texttt{metadata} input is set to the value of its ONNX initializer.}
    \label{fig:initialized-input}
\end{figure}

It is important to note that:
\begin{enumerate}
    \item The \texttt{initialized} keyword is only valid for inputs that have a corresponding initializer in the ONNX model. It is the responsibility of the solver to report an error if no such initializer exists.
    \item An initialized input variable may still be referenced in assertions. This allows the user to express constraints that relate the concrete initializer value to other variables.
    \item The \texttt{initialized} keyword does not affect the order in which inputs are matched to the ONNX model's inputs. The matching is still determined by declaration order.
\end{enumerate}
...
\vnnlib{} supports defining multiple networks by including multiple network declarations in the same query. Figure~\ref{fig:multiple-networks} 
...
\subsubsection{Zero-dimensional tensors}

It is permitted to declare zero-dimensional network nodes, i.e. a node that processes scalar values. 
...
Comments in \vnnlib{} are denoted by a semicolon (\texttt{;}) and extend to the end of the line. Whitespace in \vnnlib{} is used to separate tokens and improve readability and can include spaces, tabs, and newlines. Additional whitespace beyond the minimal necessary to separate tokens is ignored.

\section{Scoping}
\label{sec:scoping}

The scoping of variables in \vnnlib{} is extremely simple:
\begin{enumerate}
    \item All variables must be declared by a network declaration before they are used.
    \item All variable names for networks and nodes must be unique.
\end{enumerate}

\section{Typing}
\label{sec:typing}

This section describes the typing judgements for \vnnlib{} queries. The rules provide a formal description of the constraints on the syntax that were described informally in Section~\ref{sec:syntax}. 

Crucial to the typing judgements is the notion of the \emph{typing context} $\assertCtx$ which is a mapping from network names to the corresponding network declarations.
As one would expect, queries are typed by first constructing the context $\assertCtx$ by iterating over the network declarations, and then typing the globally-scoped assertions under $\assertCtx$:
\input{diagrams/typing-rule-query.tex}\unskip
The following sections describe the typing judgements for network declarations and assertions.

\subsection{Network Declarations}

A judgement that a list of network declarations $\networkDeclSet$ are well-typed in context $\assertCtx_1$ and produces a final context $\assertCtx_2$ is written as $\assertCtx_1 \vdash \networkDeclSet : \assertCtx_2$ and is defined recursively as:
\input{diagrams/typing-rule-networks.tex}\unskip
where $\nilList$ and $::$ represent the standard nil and cons constructors for building up a list.
In particular, the empty list of the networks is always well-typed under any context and returns the context unaltered. A non-empty list of networks is well-typed if the head of the list can be typed under the current context, and the tail is well-typed under the current context extended by the head of the list.

A judgement that a network is well-typed is defined as follows:
\input{diagrams/typing-rule-network.tex}\unskip
where the only requirement is that the equivalence statement is well-typed. 
There are three possibilities for the equivalence statement. Firstly, if there is no equivalence statement then it can be judged to be trivially well-typed. 

Next, if the network is marked as equal to some other network in the context then both the element types and shapes of the input and output declarations for the two networks have to match:
\input{diagrams/typing-rule-network-equal.tex}\unskip
Equal networks do not necessarily need to declare the same set of hidden nodes, however the element types and shape of any hidden node declarations that do share a common name must match. The last constraint is that the matching network in the context must not contain an equivalence statement.

Finally, if the network is marked as isomorphic to some other network in the context then the shapes, but not the element types, of the input and output declarations for the two network have to match:
\input{diagrams/typing-rule-network-iso.tex}\unskip
However, there are no constraints on hidden node declarations as although there must necessarily be a one-to-one mapping between the internal nodes of the two networks, in practice those pairs of nodes may be assigned different names in the two models.
As with equal networks, the matching isomorphic network in the context must not contain an equivalence statement.

\subsection{Assertions}

Assertions are type-checked within some variable context.
\input{diagrams/typing-rule-assertion.tex}\unskip
...
\input{diagrams/typing-rule-boolExpr.tex}\unskip
...
\input{diagrams/typing-rule-arith-ops.tex}\unskip
Note that although some ONNX types can be safely coerced between in certain cases (e.g. a \inlinevnn{float16} can be added safely to a \inlinevnn{float32} if the expected type of evaluated expression is a \inlinevnn{float32}), the standard does not permit such coercions.
All sub-expressions within an arithmetic expression must have the same type. 

The typing rules for constants defers to the typing rule provided by the underlying network theory~$\networkTheoryVar$:
\input{diagrams/typing-rule-arith-constants.tex}\unskip
...
\input{diagrams/typing-rule-arith-variables.tex}\unskip
Crucially, therefore variables provide the source of typing for the whole arithmetic expression. 
One consequence of this is that comparisons that contain no variables (e.g. \texttt{(<= 0.0 1.0)}) have no canonical type and therefore are not typable.

\subsection{Models}
\label{sec:model-typing}

The input to a solver consists of a query~$\queryVar$ and a mapping:
\begin{equation*}
    \networkImplementationSet : \mgrammar{<name>} \rightarrow \mgrammar{<model>}\networkTheoryVarParam
\end{equation*}
which associates the name of each network declaration in $\queryVar$ with a corresponding model. 
This section describes a series of judgements that formalise the notion of the mapping~$\networkImplementationSet$ being well-formed with respect to the networks declared by~$\queryVar$. 

To start with, a mapping~$\networkImplementationSet$ is well-formed with respect to a query~$\queryVar$ if it can be judged to be well-formed with respect to the list of network declarations. 
\input{diagrams/typing-rule-model-query.tex}\unskip
A list of models is judged to be compatible with a list of network declarations if each model is compatible with the corresponding network declaration:
\input{diagrams/typing-rule-model-nil-cons.tex}\unskip
An individual model $\networkImplementation$ is compatible with a network declaration $\networkDeclVar$ if (i) it is compatible with the equivalence statement, (ii) it can be typed according to the network declaration and (iii) it has hidden nodes of the correct type: 
\input{diagrams/typing-rule-model-network.tex}\unskip
where the function $\networkDeclType : [\mgrammar{<input>}] \times [\mgrammar{<output>}] \rightarrow \mgrammar{<modelType>}\networkTheoryVarParam$ maps the list of input and output declarations to the overall type of the model in the obvious way.

A network model is compatible with $\texttt{isomorphic-to }\nameVar$ and $\texttt{equal-to }\nameVar$ statements if it is related to the earlier provided network model in the appropriate way:
\input{diagrams/typing-rule-model-iso.tex}\unskip
If no equivalence statement is provided, then any network implementation is compatible. Note that the equality judgment `$=$' in the $\texttt{equal-to }\nameVar$ case is \emph{not} parameterised by $\networkTheoryVar$ as it refers to absolute equality, i.e. the network models must actually be the \emph{same} model file, rather than merely being semantically equal. 

Finally a network model is compatible with a list of hidden node declarations if each hidden node declaration corresponds to an output of some node in the model of the correct type:
\input{diagrams/typing-rule-model-hidden.tex}\unskip

\subsection{Input Assignments}
\label{sec:input-assignment-typing}

As output, solvers will produce either a judgement that the query $\queryVar$ is unsatisfiable or a satisfying assignment consisting of a mapping:
\begin{equation*}
\networkInputSet : \mgrammar{<name>} \rightarrow \mgrammar{<tensor>}\networkTheoryVarParam
\end{equation*}  
which associates the name of each input declaration in $\queryVar$ with a corresponding syntactic tensor. 

To start with, a mapping~$\networkInputSet$ is well-formed with respect to a query~$\queryVar$ if it can be judged to be well-formed with respect to all the input declarations in $\queryVar$. 
\input{diagrams/typing-rule-inputAssignment-query.tex}\unskip
Using the typing judgement \textsc{Tensor} from the underlying network theory in Figure~\ref{fig:onnx-signature}, the following typing judgment $\vdash \networkInputSet : \queryVar$ ensures that the input assignment~$\networkInputSet$ is well typed with respect to the network declarations in the query~$\queryVar$:
\input{diagrams/typing-rule-inputAssignment-inputDecl.tex}\unskip

\section{Semantics}
\label{sec:semantics}

This section will now define the semantics of the \vnnlib{} query language, i.e. a precise description of the mathematical satisfiability problem that a solver is attempting to answer when provided with a well-typed query $\queryVar$ and set of models~$\networkImplementationSet$.
There are various applications of the semantics of the language, including:
\begin{itemize}
\item \textbf{Proving the soundness of query transformations}, i.e. that transformations of a query preserves the meaning (e.g. conversion of assertions into Disjunctive Normal Form (DNF) do not change the satisfiability of the query). 
\item \textbf{Proving the soundness of solvers}, i.e. that a particular solver will give always give the right answer.
...
In order to determine whether a query $\queryVar$ over a set of network models $\networkImplementationSet$ is satisfied by a given input assignment $\networkInputSet$, it is necessary to first calculate the values of the declared network variables. 
The values are captured formally by the \emph{environment}~$\assertEnv$ which is a function:
\begin{equation*}
\assertEnv : (\nameVar : \tensorTypeVar) \rightarrow \semTensorTheory{\delta}
\end{equation*}
mapping each name $\nameVar$ of the declared input/hidden/output network variable of type $\tensorTypeVar$ to a mathematical tensor of the corresponding type.

Given a list of network declarations $\networkDeclSet$, a set of models $\networkImplementationSet$ and an input assignment $\networkInputSet$, the environment $\assertEnv (\networkDeclSet, \networkImplementationSet, \networkInputSet)$ is calculated as the union of the environments generated by each network declaration:
\begin{equation*}
...
\begin{align*}
    ...
    \\  
    \assertEnv_{\outputDeclSet}(\outputExpansion, \onnxNameVar) 
    &= \{ \nameVar \rightarrow \semModelTheory{\networkImplementation}(\sem{\networkInput}, \onnxNameVar) \}
\end{align*}}
...
\input{diagrams/semantics-assertion.tex}\unskip
There is nothing particularly surprising about the semantics of boolean expressions which are interpreted as boolean values:
\input{diagrams/semantics-boolExpr.tex}\unskip
The only thing to note is that the exact semantics of comparisons is delegated to that of the underlying network theory $\networkTheoryVar$ to ensure correct behaviour for e.g. floating point comparisons.

The semantics of arithmetic expressions, which are interpreted as zero-dimensional tensors, is defined as follows:
\input{diagrams/semantics-arithExpr.tex}\unskip
...
The semantics of a query is formally defined as a function. The input to the function is a well-typed query $\queryVar$ and a set of syntactic models $\networkImplementationSet$ from the underlying theory $\networkTheoryVar$ that are well-typed with respect to $\queryVar$. The output of the function is the truth value of whether there exists a input assignment consisting of a syntactic list of tensors $\networkInputSet$ from the underlying theory that satisfies all the assertions concurrently:
\begin{equation*}
\semQuery{\queryExpansion} = (\networkImplementationSet : \mgrammar{<model>}_\networkTheoryVar^+) \rightarrow
    \exists~(\networkInputSet : \mgrammar{<tensor>}_\networkTheoryVar^+). \; \bigwedge_{\assertionVar \in \assertionSet}~ 
    \semApp{assert}{\assertionVar}^{\assertEnv(\networkDeclSet, \networkImplementationSet,\networkInputSet)} \\
\end{equation*}
Or less formally: given a set of networks does there exist an input assignment under which all the assertions to evaluate to true?
A query is \textit{satisfiable} if such an assignment exists, and \textit{unsatisfiable} otherwise.
...
\begin{figure}[t]
    \newcommand{\realParam}{\hspace{-0.25em}$^\real$}
    \centering
    ...
    \begin{subfigure}{\textwidth}
		\input{diagrams/real-network-theory-grammar.tex}
		\caption{Type-system for real network model expressions.}
		\label{fig:real-theory-grammar}
	\end{subfigure}
    \vspace{0.5em}
    
    \begin{subfigure}{\textwidth}
		\input{diagrams/real-network-theory-semantics.tex}
		\caption{Semantics for real network model expressions. The semantics for the comparison and arithmetic operations are simply their real-valued interpretations.}
		\label{fig:real-theory-semantics}
	\end{subfigure}
    \caption{Construction of a real-valued network theory $\networkTheoryVar_\real$ from an abstract network theory $\networkTheoryVar$ and additional function $\semApp{realModel}{\cdot}\networkTheoryVarParam$ that returns the semantics of a network model as a function over real numbers.}
    \label{fig:real-network-theory}
\end{figure}

The notion of a network theory gives us a modular way of defining the syntax and semantics of a real-valued query.
Given a network theory $\networkTheoryVar$ and a function $\sem{\mathtt{realModel}}\networkTheoryVarParam$ that interprets models in $\networkTheoryVar$ as real-valued functions, we can define a new network theory $\networkTheoryVar_\real$ as shown in \autoref{fig:real-network-theory}. In particular:
\begin{itemize}
...
\end{itemize}
We can then use this new theory $\networkTheoryVar_\real$, to obtain both the syntax and semantics for queries over $\real$ simply by reparametrising the exsting syntax and semantics defined in the previous sections. 
