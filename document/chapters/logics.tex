...
Concretely, given a network theory $\networkTheoryVar$ and a logic $\logicVar \in \logicSet$, a solver for that logic is a function with the following type:
\begin{equation*}
\text{Solver}^{\logicVar}_{\networkTheoryVar} = \forall \queryVar \in \logicVar, \networkImplementationSet . \semQuery(\queryVar, \networkImplementationSet)
\end{equation*}