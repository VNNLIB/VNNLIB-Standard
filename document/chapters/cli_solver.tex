\chapter{Solver Command-line Interface}
...
{Instructs the solver to output satisfying assignments in the seralised format described in Section~\ref{sec:seralised-assignment-format}. Solvers are only required to support this argument if they declare support for serialising assignments as described in Section~\ref{sec:other-capabilities}.}
{-{}-serialise-assignments /path/to/output}

\clOption
{-{}-unsat-proofs}
{When provided, this argument instructs the solver to produce a proof of unsatisfiability in the \vnnlib{} proof format, whenever the query is found to be unsatisfiable. Solvers are only required to support this argument if they declare support for UNSAT proof generation as described in Section~\ref{sec:other-capabilities}.} { -{}-unsat-proofs /proof/file/path}
...
\clOutputOption
{-{}-onnx-operators-unsat-proofs}
{Reports the ONNX operators (e.g., \texttt{Conv}, \texttt{Relu}, \texttt{Gemm}) that are supported by the solver when producing proofs of UNSAT. The output should be a subset of the ONNX operators reported via the \texttt{supports -{}-onnx-operators} command. Solvers are only required to support this argument if they declare support for UNSAT proof generation as described in Section~\ref{sec:other-capabilities}.
}
{A newline-separated list of lines, where each line contains the name of an ONNX operator followed by a possibly empty space-separated list of ONNX element types.  If the list is empty, then the solver is assumed to support the operator for all element types it reports via the \texttt{supports -{}-onnx-element-types} command.
}
\begin{lstlisting}[style=bash]
...
\clOutputOption
{-{}-unsat-proofs}
{Can the verifier generate proofs of unsatisfiability?} 
