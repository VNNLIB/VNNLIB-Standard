...
\item \textsc{(ModelHidden)} - a judgment that a model~$\modelVar$ has a hidden node called~$\nameVar$ whose output is a tensor of type~$\tensorTypeVar$.
...
\item $\semModelHiddenAbs$ - a function mapping a syntactic model $\tensorVar$ of type $\modelTypeVar$ and the name of a hidden node $\nameVar$ of type $\tensorVar$ to the mathematical function that maps a list of input mathematical tensors to a single output mathematical tensor that represents the output of that hidden node.
