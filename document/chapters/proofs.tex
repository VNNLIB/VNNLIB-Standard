...
\begin{enumerate}
	\item \textbf{Network assumptions:} Introduce formulas assumed to be correct by the structure of the neural networks. To ease proof checking, \vnnlib{} proofs specify the exact source of each network assumption depending on the context of their appearance:

	\begin{itemize}	
	\item The \texttt{assume-network} command specifies a \emph{network assumption context}. This command must name a network used in the \vnnlib{} specification file and it defines that network's context. This context is then used to define additional node assumptions, imported from the declared network.
	\item The \texttt{assume-node} command specifies a \emph{node assumption context}. This command is defined within a network assumption context, and is used to import of assumptions from a specific ONNX node within that network. The declaration must include the name of the ONNX node and its type.
	\item Within a node context, the \texttt{assume} command specifies a constraint imported from that node (within some neural network). These commands are pure Alethe assumption, but they are required to comply with the structure defined by their  context, as described in Section~\ref{sec:proof-node-encoding}.
	\end{itemize}
	
	\item \textbf{Query assumptions:} Introduce formulas imported directly from the \vnnlib{} query specification, outside any network context. These assumptions are also introduced using the \texttt{assume} command and must reproduce formulas exactly as stated in the specification. 

	All \texttt{assume} commands contain a unique identifier name and the imported formula. Additionally, the requirement to list the importing node requires all ONNX networks used in this format to contain a unique name for each node.

	The proof checker \emph{must} certify that all network assumptions are correctly imported from a valid node in the network, and that the variable names are consistent across all assumptions and with the ONNX files. All query assumptions must match an assertion defined in the \vnnlib{} specification file, and have consistent variable names with the other assumptions.
...
Linear equations are typically used to encode the affine transformations induced by the internal linear nodes of the network, as well as linear constraints originating from the specification.
...
In particular, consider a linear equation that computes the value of a neuron from the values of neurons in a preceding node. Let $X_k$ be a node and $i_k$ be an index vector such that $X_k[i_k]$ denotes a neuron in that node. Similarly, consider $m$ neurons from a preceding node $X_j$, identified by the index vectors $i_1,\dots,i_m$, namely
$X_j[i_1],\dots,X_j[i_m]$.
Then, the computation: 
\[X_k[i_k] = c_1 \cdot X_j[i_1] + \cdots + c_m \cdot X_j[i_m] + b\] can be written as the assumption:

\begin{code}[style=lbnf]
(assume e (= Xk_ik (+ (* c1 Xj_i1) ... (* cm Xj_im) b)))
\end{code}


\subsubsection{ONNX Nodes Encoding}
\label{sec:proof-node-encoding}
...
To represent a fully connected node, $B$ and $C$ are constant Tensors, resulting in
...
\begin{code}[style=lbnf]
(assume-node some_node leaky_relu
    (assume lr (ite (>= y 0.0) 
                    (= y x) 
                    (= y (* a x)))) 
)                    
...
To represent a linear mode, $B$ is a constant Tensor, resulting in linear equations over the network variables of $A$.
Using the above formulation, we can assume the following equations used to derive the output variable $Y[q,i,j]$:

\begin{code}[style=lbnf]
(assume-node some_node matmul
    ...
...
For example, the notations above allows us to formalize any bounds on the form $l_j \leq X_j[i_j] \leq u_j$ for some numeral elements $l_j,u_j$ as:
...
Consider the neural network and \vnnlib{} specification illustrated in Figure~\ref{fig:proof-query}. Assume that the matrix-multiplication nodes defined by the ONNX file are as follows:
...
Finally, assume that the input node operator is named \texttt{il}, the hidden ReLU node operator is named \texttt{hr}, and the output node operator name is \texttt{ol}
...
An example proof of unsatisfiability for this query is shown in Example~\ref{lst:proof}.
The proof introduces two variables for each hidden neuron in the ReLU node \texttt{hidden}: \texttt{H\_0\_i\_b} and \texttt{H\_0\_i\_f} corresponding to the variables described in Section~\ref{sec:proof-syntax}.
The proof begins by declaring the network assumption in lines 2--20, which includes the equations induced by the input and output nodes of the network (lines 3--8, 9--1, respectively), followed by assumptions encoding the ReLU activation node (lines 12--19). Then, in lines 23--29, the proof proceeds by importing bound assumption directly from the \vnnlib{} specification file.
Finally, lines 32--69 contain a sequence of derivation steps that lead to the empty clause, thereby establishing unsatisfiability. \newline


\begin{lstlisting}[style=lbnf,basicstyle=\ttfamily\scriptsize\bfseries,linewidth=1.05\textwidth]
	; Network assumptions
	<@(assume-network encoder
	      (assume-node il matmul@>
	          (assume e1 (!(= (+ X_0_0 (- X_0_1 ) (- H_0_0_b)) 0.0)
	                     :named e1))
	          (assume e2 (!(= (+ (* 2.0 X_0_1) X_0_2 (- H_0_1_b)) 0.0)
	                      :named e2))
	      <@)@>
	      <@(assume-node ol matmul@>
	          (assume e3 (!(= (+ H_0_1_f (- H_0_0_f) (- Y_0_0)) 0.0):named e3))
	      <@)@>
	      <@(assume-node hr relu@>
	          (assume r1 (!(ite (>= H_0_0_b 0.0)(= H_0_0_b H_0_0_f)(<= H_0_0_f 0.0))
	                     :named r1))
	          (assume r2 (!(ite (>= H_0_1_b 0.0)(= H_0_1_b H_0_1_f)(<= H_0_1_f 0.0))
	                     :named r2))
	          (assume lf0 (!(>= H_0_0_f 0.0):named lf0))
	          (assume lf1 (!(>= H_0_1_f 0.0):named lf1))
	       <@)@>
	<@)@>
...
\end{lstlisting}
\captionof{lstlisting}{An example for a \vnnlib{} proof in Alethe. The text highlighted in red is an addition on top of the standard Alethe.}
\label{lst:proof}
