...
This chapter aims to unify proofs of unsatisfiability of neural network verification queries into a single expressive format. The goal of this standardization is to modularly separate verifiers from proof checkers, enabling easier comparison between verifiers and checkers, and fostering the independent development of both.
...
\begin{code}[style=lbnf]
	(assume_n e (= d (+ (* c1 x1) ... (* ck xk))) :layer ...)
\end{code}
...
\begin{code}[style=lbnf]
	(assume_n e (= Xk[ik] (+ (* c1 Xj[i1]) ... (* cm Xj[im]) b))  :layer ...)
\end{code}
...
The representation of each activation function requires its own formalization in SMT, which can be interpreted in Alethe. We list here the formalization of several common activation functions in alphabetical order. The formalization of linear operators is straightforward using linear equations. To ease notation, we use here a simple naming convention for variables, which denotes the input and output variables of the constraint as $b$ and $f$, respectively.
...
\begin{code}[style=lbnf]
(assume_n is_int f :layer ...)
(assume_n (< f (+ b 1)) :layer ...)
(assume_n (>= f b) :layer ...)
\end{code} 
...
\begin{code}[style=lbnf]
(assume_n sig (= f (sgm b)) :layer ...)
(assume_n t (= f (tanh b)) :layer ...)
\end{code}
...
\begin{figure}[p]
	...
	(assume_n lf0 (!(>= H_0_0_f 0.0):named lf0):layer hr)
	(assume_n lf1 (!(>= H_0_1_f 0.0):named lf1):layer hr)
