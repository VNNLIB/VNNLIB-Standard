...
In this chapter, we elaborate on a format for unsatisfiability proofs for neural network verifiers, based on Alethe. The proof is intended to be written in a dedicated file that accompanies the  \vnnlib{} specification and the ONNX network(s) files. The subset of Alethe rules used in this chapter is tentative, and we welcome any integration of additional rules, useful for verification of neural networks.
...
\begin{enumerate}
	\item \textbf{Assumptions:} Introduce formulas assumed to be correct. These can either be imported from the neural network or the \vnnlib{} specification being proved, or be assumed correct at a given subproof context.
...
\begin{code}[style=lbnf]
	(assume e (= Xk[ik] (+ (* c1 Xj[i1]) ... (* cm Xj[im]) b)))
\end{code}
...
The constraint $Y_k[i_k] = \mathrm{ReLU}(X_j[i_j])$ is formulated as
follows: 

\begin{code}[style=lbnf]
	(assume r (ite (>= Xj[ij] 0.0) 
	               (= Yk[ik] Xj[ij]) 
	               (<= Yk[ik] 0.0))) 
\end{code}
