...
\begin{enumerate}
	\item \textbf{Assumptions:} Introduce formulas assumed to be correct. These can either be imported from the neural networks or the \vnnlib{} specification being proved. To ease proof checking, \vnnlib{} proofs specify the source of each assumption depending on the context of their appearance.

	\begin{itemize}	
	\item The \texttt{assume-network} command specifies a \emph{network assumption} environment. This command must name a network used in the \vnnlib{} specification file and it defines a network context. This context is then used to define additional assumptions, imported from the declared network.
	\item The \texttt{assume} command specifies a constraint imported from either a neural network or the  \vnnlib{} specification file. If the command appears within an \texttt{assume-network} context, then it represents an assumption imported from that network and must explicitly state the name of the ONNX layer used for its import, by using the \texttt{:layer} keyword. Otherwise, it directly imports assertions from the \vnnlib{} specification file, and should not use the \texttt{:layer} flag. 
	\item Additional assumptions can be assumed correct at a given subproof context (see below in Section~\ref{sec:subproofs}).
	\end{itemize}
	...
	For all assumptions declared outside an \texttt{assume-network} command, the \texttt{:layer} flag can either lead to a checking failure, raise a warning, or simply be disregarded.
...
\begin{code}[style=lbnf]
(assume e (= Xk[ik] (+ (* c1 Xj[i1]) ... (* cm Xj[im]) b))     :layer ...)
\end{code}
...
\begin{code}[style=lbnf]
(assume sig (= f (sgm b)) :layer ...)
(assume t (= f (tanh b)) :layer ...)
\end{code}
...
The proof begins by declaring the network assumption, which includes the equations induced by the linear layers of the network, followed by assumptions encoding the ReLU activation constraints. Then, the proof proceeds by importing bound assumption directly from the \vnnlib{} specification file.
