...
def test_multiple_operand_mismatches():
    ...
    json_error = json.loads(str(exc_info.value))
    assert len(json_error["errors"]) == 2  # Reports both type mismatches
    
    # Should report both Z (int16 vs int8) and 3.14 (float vs int) mismatches
    assert all("TypeMismatch" in error["errorCode"] for error in json_error["errors"])
    offending_symbols = {error["offendingSymbol"] for error in json_error["errors"]}
    assert "Z" in offending_symbols
    assert "3.14" in offending_symbols
